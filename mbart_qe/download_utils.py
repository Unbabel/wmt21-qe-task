...
PRETRAINED_MODEL = {
    "wmt21-mbart-m2": "https://unbabel-experimental-models.s3.amazonaws.com/wmt21/wmt21-mbart-m2.tar.gz"
}
...
def get_cache_folder():
    if "HOME" in os.environ:
        cache_directory = os.environ["HOME"] + "/.cache/torch/"
...
def download_mbart_qe(model: str, saving_directory: str = None) -> str:
    """
    ...
    elif model not in PRETRAINED_MODEL.keys():
        raise Exception(
            f"{model} is not in the `availale_metrics` or is a valid checkpoint folder."
        )

    elif PRETRAINED_MODEL[model].startswith("https://"):
        download_file_maybe_extract(
            PRETRAINED_MODEL[model], directory=saving_directory
        )
    ...
    checkpoints_folder = saving_directory + model
    checkpoints = [
